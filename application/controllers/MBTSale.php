...
class MBTSale extends MY_Controller {
	public $table = 'tbl_master_branch_sale';
	...
	public function index()
	{
		...
		$template['body'] = 'MBTSale/list';
		$template['script'] = 'MBTSale/script';
		$this->load->view('template',$template);
	}
	public function add(){
	   
		$template['tax_names'] = $this->MBTSale_model->view_by();
		$this->form_validation->set_rules('branch_name', 'Branch Name', 'required');
		if ($this->form_validation->run() == FALSE) {
			$admno = $this->MBTSale_model->get_admno();
			if(isset($admno->sale_id)){$adm=$admno->invoice+1;}else{$adm=1;}
			$template['adm'] = $adm;
			$template['body'] = 'MBTSale/add';
			$template['script'] = 'MBTSale/script2';
		...
		if(isset($finyear[0]->fin_year)){$fin=$finyear[0]->fin_year;}else{$fin=0;}
			...
			$branch_id_fk = $this->input->post('branch_name');
			 for($i=0;$i<$temp;$i++){
				...
				$bresult = $this->Branch_transfer_model->getbstock($product_id_fk[$i],$branch_id_fk);
				//var_dump($bresult);die;
				if($bresult)
				{
					$bstok = $this->General_model->get_row_bstock($product_id_fk[$i],$branch_id_fk);
					$updated_bstk = intval($bstok->product_stock) + intval($sale_quantity[$i]);
					$bstok_array = ['product_stock' => $updated_bstk];
					$result = $this->General_model->updat('tbl_product',$bstok_array,'bproduct_id_fk',$product_id_fk[$i],'branch_id_fk',$branch_id_fk);
					$response_text = 'Branch_transfer added  successfully';
				}
				else
				{
						$datas = array(
						'bproduct_id_fk' => $product_id_fk[$i],
						'branch_id_fk' => $this->input->post('branch_name'),
						'product_name' => $product_name[$i],
						'product_unit' => $product_unit[$i],
						'product_hsn' => $hsncode_id[$i],
						'product_hsncode' => $hsn[$i],
						'product_stock' => $sale_quantity[$i],
						'product_created_date' => $sale_date,
						'product_status' => 1,
				 ...
				 $result2 = $this->General_model->add('tbl_product',$datas);
						...
						$data = array(
								...
								'sale_date' => $sale_date,
								'sale_branch_id_fk'=>$this->input->post('branch_name'),
								'sale_status' => 1
									);
	       ...
	       redirect('/MBTSale/invoice/'.$invoice_no, 'refresh');
		}
	}
	public function invoice($invoice_no)
	{
		$template['body'] = 'MBTSale_Invoice/Invoice2';
		$template['script'] = 'MBTSale_Invoice/script';
		
		$template['records'] = $this->MBTSale_model->get_invc($invoice_no);
		$this->load->view('template', $template);
	}
	public function invoiceview($invoice_no)
	{
		...
		$template['records'] = $this->MBTSale_model->get_invoice($invoice_no);
	...
	public function get(){
       ...
       // $sessid = $this->session->userdata['id'];
		//$shopid = $this->MBTSale_model->get_shop($sessid);
		//if(isset($shopid[0]->shop_id_fk)){$shid=$shopid[0]->shop_id_fk;}else{$shid=0;}
		//$param['shop'] =$shid;
		$data = $this->MBTSale_model->getSaleReport($param,$branch_id_fk);
	...
	/* public function delete(){
        ...
        echo $data_json;
		redirect('/MBTSale/', 'refresh');
    } */

	public function delete()
	{
		$invoice_number = $this->input->post('invoice_id');
		$records = $this->MBTSale_model->get_invc($invoice_number);
		for($i=0; $i< count($records); $i++)
		{
			$stok = $this->MBTSale_model->get_prodstk($records[$i]->product_id);
			//var_dump($stok);die;
	...
	$result = $this->MBTSale_model->getproductnum();
	echo json_encode($result);
    }
    public function getproductname(){
	header('Content-Type: application/x-json; charset=utf-8');
	$result = $this->MBTSale_model->getproductnames();
	echo json_encode($result);
    }
    public function getprice()
	{
	$product_id_fk = $this->input->post('product_id_fk');
	$data = $this->MBTSale_model->getprice($product_id_fk);
	$json_data = json_encode($data);
	echo $json_data;
	}
	// function get_price(){
	...
	// print_r($product_id);
	// exit();
	// $result = $this->MBTSale_model->get_price($product_id);
	// echo json_encode($result);
 //    }
	...
	function gettax(){
	header('Content-Type: application/x-json; charset=utf-8');
	$result = $this->MBTSale_model->gettax();
	echo json_encode($result);
    }
	public function get_purchasedetails(){
	    $product_num = $this->input->post('product_num');
		//$product_size = $this->input->post('product_size');
		//$data = $this->MBTSale_model->get_purchasedetails($product_num,$product_size);
		$data = $this->MBTSale_model->get_purchasedetails2($product_num);
	...
	public function getstock(){
		...
		$stok = $this->MBTSale_model->get_stok($product_id,$shop_id);
	...
	public function getproduct()
	{
		...
		$product_size = $this->input->post('product_size');
		$data = $this->MBTSale_model->getproduct($product_cmpny,$product_size);
		$data_json = json_encode($data);
		echo $data_json;
    }
	public function getproductcompany()
	{
		header('Content-Type: application/x-json; charset=utf-8');
		$product_size = $this->input->post('product_size');
		$data = $this->MBTSale_model->getproductcompany($product_size);
		$data_json = json_encode($data);
		echo $data_json;
	}
	 public function getproductname1(){
    $prod_id = $this->input->post('p_id');
	$data['product'] = $this->MBTSale_model->getproductname1($prod_id);
	echo json_encode($data);
    }
	// function get_memberaddress(){
	// 	$id = $this->input->post('member_id');
	// 	$data = $this->MBTSale_model->get_memberaddress($id);
	// 	echo json_encode($data);
	// }
	function get_memberaddress(){
		...
		//$data = $this->MBTSale_model->get_memberaddress($id);
		//echo json_encode($data);
		$records = $this->MBTSale_model->get_memberaddress($id);
	...
	function get_phone(){
		...
		//$data = $this->MBTSale_model->get_memberaddress($id);
		//echo json_encode($data);
		$records = $this->MBTSale_model->get_phone($id);
		$data_json = json_encode($records);
        echo $data_json;
	}
	...
	{
		$id = $this->input->post('id');
		$data = $this->MBTSale_model->getOldBalanceDetails($id);
		echo json_encode($data);
	}
	public function getMemberList()
	{
		$mem_id = $_POST['mem_id'];
		$data = $this->MBTSale_model->getMemberlists($mem_id);
		echo json_encode($data);
	}

	public function get_table(){
		$table='tbl_production_itemlist';
		$result=$this->MBTSale_model->get_table($table);
	...
	{$branch_id_fk=$this->session->userdata('branch_id_fk');
		...
		$data =  $this->MBTSale_model->getproduct_names($id,$branch_id_fk);
		echo json_encode($data);
	}
	...
	public function getProductDetails1()
	{
		...
		$data['product_name1'] =  $this->MBTSale_model->get_row_barcode($p_name);
		$data['product_name2'] =  $this->MBTSale_model->get_row_barcode_branch($p_name,$branches_id);
		...
		$prod1['product_hsn'] = $data['product_name2']->product_hsn;
		$prod1['product_unit'] = $data['product_name2']->product_unit;
		$prod1['product_category'] = $data['product_name2']->product_category;
		$prod1['product_unit_type'] = $data['product_name2']->product_unit_type;
	...
	public function getProductDetails2()
	{
		...
		$data['product_name1'] =  $this->MBTSale_model->get_row_rate($pid);
		if($ptype==1)
		{
	...
	{
		$records = $this->MBTSale_model->get_sale_pdf($auto_invoice);
	...
	public function get_old_bal()
	{
		...
		$records = $this->MBTSale_model->get_old_bal($mem_id);
		$data_json = json_encode($records);
		echo $data_json;
	}
	...
	public function getSaleReturn(){
		...
		$data = $this->MBTSale_model->getSaleReturnReport($param);
		$json_data = json_encode($data);
    	echo $json_data;
    }
	...
	public function editedSaleRet($auto_invoice)
	{
		...
		$template['records'] = $this->MBTSale_model->getEditData($auto_invoice);
		$this->load->view('template', $template);
	}
	...
	public function editReturnSale2()
	{
		...
		for ($i = 0; $i < $temp; $i++) {
			...
			if(isset($sale_idss[$i])){
				...
				$stok[$i] = $this->MBTSale_model->get_stoks($product_id_fk[$i]);
		...
		redirect('/MBTSale/SaleReturn/', 'refresh');
	}
