...
var response = $("#response").val();
  ...
  $table = $('#product_table').DataTable( {
        ...
        "bDestroy" : true,
        "ajax": {
            "url": "<?php echo base_url();?>Items/getProducts2/",
            "type": "POST",
            "data" : function (d) {
        ...
        $('#product_material').change(function() {
          //  msg = 'dist_id=' + $(this).val();
            ...
            if (product_type) {
                $.ajax({
                    type: 'POST',
                    url: '<?php echo base_url() . 'index.php/Products/getprice'; ?>',
                    data: {
                        product_type: $('#product_material').val()
                    },
                    success: function(data) {
                        $('#product_meter_price').html(data);
                        $('#product_meter_price').val(data);
                    },
                    error: function() {}
                });
            }
