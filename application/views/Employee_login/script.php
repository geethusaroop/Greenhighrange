<script>
  ...
  $(function () {
    ...
    $table = $('#employee_table').DataTable( {
        ...
        "bDestroy" : true,
        "ajax": {
            "url": "<?php echo base_url();?>Employee_login/get",
            "type": "POST",
            "data" : function (d) {
      ...
      if(data['emp_designation']==1)
         {
          $('td', row).eq(2).html('Office Assistant');
         }
         else if(data['emp_designation']==2)
         {
          $('td', row).eq(2).html('Sales Executive');
         }
         else if(data['emp_designation']==3)
         {
          $('td', row).eq(2).html('Technician');
         }
      
			$('td', row).eq(5).html('<center><a href="<?php echo base_url();?>Employee_login/edit/'+data['id']+'"><i class="fa fa-edit iconFontSize-medium" ></i></a>&nbsp;&nbsp;&nbsp;<a onclick="return confirmDelete('+data['id']+')"><i class="fa fa-trash-o iconFontSize-medium" ></i></a></center>');
		},
        "columns": [
            { "data": "user_status", "orderable": true },
            { "data": "emp_fname", "orderable": false },
  ...
  function confirmDelete(id){
    var conf = confirm("Do you want to Delete ?");
        if(conf){
        $.ajax({
            url:"<?php echo base_url();?>Employee_login/delete/",
            data:{id:id},
            method:"POST",
            datatype:"json",
  ...
  {
    $.ajax({
            url:"<?php echo base_url()?>Employee_login/getdes",
            type: 'POST',
            data: {id:id},
            dataType: 'json',
            success:
