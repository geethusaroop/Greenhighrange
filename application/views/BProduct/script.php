...
$(function () {
  ...
  $table = $('#classinfo_table').DataTable( {
    ...
    "createdRow": function ( row, data, index ) {
     ...
     $('td', row).eq(9).html('<center><a href="<?php echo base_url();?>index.php/BProduct/edit/'+data['product_id']+'"><i class="fa fa-edit iconFontSize-medium" ></i></a> &nbsp;&nbsp;&nbsp;<a onclick="return confirmDelete('+data['product_id']+')"><i class="fa fa-trash-o iconFontSize-medium" ></i></a></center>');
    },
    "columns": [
...
function confirmDelete(product_id){
  ...
  if(conf){
    $.ajax({
      url:"<?php echo base_url();?>BProduct/delete",
...
$(document).on('change','#product_hsn',function(){
  ...
  {
    $.ajax({
            url:"<?php echo base_url()?>BProduct/gethsncode",
            type: 'POST',
            data: {id:id},
            dataType: 'json',
            success:
