...
$(document).on('change','#vouch_name',function(){
  ...
  $(function () {
    ...
    $table = $('#receipt_list').DataTable( {
        ...
        "bDestroy" : true,
        "ajax": {
            "url": "<?php echo base_url();?>BCustomer_receipt/get",
            "type": "POST",
            "data" : function (d) {
      ...
      $('td', row).eq(6).html('<center><a href="<?php echo base_url();?>Customer_receipt/receipt/'+data['receipt_id']+'"><i class="fa fa-file iconFontSize-medium" ></i></a></center>');
			$('td', row).eq(7).html('<center><a href="<?php echo base_url();?>BCustomer_receipt/edit/'+data['receipt_id']+'"><i class="fa fa-edit iconFontSize-medium" ></i></a>&nbsp;&nbsp;&nbsp;<a onclick="return confirmDelete('+data['receipt_id']+','+data['receipt_member_id_fk']+','+data['receipt_amount']+')"><i class="fa fa-trash-o iconFontSize-medium" ></i></a></center>');
        },
 ...
 function confirmDelete(receipt_id,receipt_member_id_fk,receipt_amount){
    ...
    if(conf){
        $.ajax({
            url:"<?php echo base_url();?>BCustomer_receipt/delete",
