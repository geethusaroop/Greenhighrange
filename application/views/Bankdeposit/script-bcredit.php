...
$(document).on('change','#vouch_name',function(){
  ...
  $(function () {
    ...
    $table = $('#receipt_list').DataTable( {
        ...
        "bDestroy" : true,
        "ajax": {
            "url": "<?php echo base_url();?>Bankdeposit/get_bcredit",
            "type": "POST",
            "data" : function (d) {
        ...
        "createdRow": function ( row, data, index ) {
			...
			$('td', row).eq(6).html('<center><a href="<?php echo base_url();?>Bankdeposit/edit_bcredit/'+data['bd_id']+'"><i class="fa fa-edit iconFontSize-medium" ></i></a>&nbsp;&nbsp;&nbsp;<a onclick="return confirmDelete('+data['bd_id']+','+data['bd_member_id_fk']+','+data['bd_amount']+')"><i class="fa fa-trash-o iconFontSize-medium" ></i></a></center>');
        },
 ...
 function confirmDelete(bd_id,bd_member_id_fk,bd_amount){
    ...
    if(conf){
        $.ajax({
            url:"<?php echo base_url();?>Bankdeposit/delete_bcredit",
