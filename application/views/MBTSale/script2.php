...
function addMore() {
	...
	$.ajax({
            ...
            //     {
            //         $.ajax({
            //               url: "<?php echo base_url();?>MBTSale/getprice",
  ...
  $(function () {
    ...
    $table = $('#sale_details_table').DataTable( {
        ...
        "createdRow": function ( row, data, index ) {
			...
			$('td', row).eq(6).html('<center><a target ="_blank"  href="<?php echo base_url();?>MBTSale/invoiceview/'+data['invoice_number']+'"><i class="fa  fa-file iconFontSize-medium" ></i></a></center>');
            },
        "columns": [
...
<script type="text/javascript">
   ...
   // element18.setAttribute("class", "democlass");
      //  element17.required = "required";
        ...
        element21.name = "hsncode_id[]";
        element21.id = "hsncode_id"+rowCount;
      ...
      var cell22 = row.insertCell(21);
        ...
        element22.name = "product_unit[]";
        element22.id = "product_unit"+rowCount;
        element22.setAttribute("size", "6");
      cell22.appendChild(element22);

      var cell23 = row.insertCell(22);
        var element23 = document.createElement("input");
        element23.type = "hidden";
        element23.name = "product_category[]";
        element23.id = "product_category"+rowCount;
        element23.setAttribute("size", "6");
      cell23.appendChild(element23);

      var cell24 = row.insertCell(23);
        var element24 = document.createElement("input");
        element24.type = "hidden";
        element24.name = "product_unit_type[]";
        element24.id = "product_unit_type"+rowCount;
        element24.setAttribute("size", "6");
      cell24.appendChild(element24);


        var options = {
        // url: "<?php echo base_url() ?>MBTSale/getproduct_names",
        url: function(phrase) {
            return "<?php echo base_url() ?>MBTSale/getproduct_names?phrase=" + phrase + "&format=json";
        },
        getValue: "name",
        list: {
            match: {
    ...
    { 
                  $.ajax({
                      type: 'POST',
                      url: '<?php echo base_url() . 'index.php/MBTSale/getProductDetails1'; ?>',
                      data: {
    ...
    { //alert($("#product_num"+row).val());
                  $.ajax({
                      type: 'POST',
                      url: '<?php echo base_url() . 'index.php/MBTSale/getProductDetails2'; ?>',
                      data: {
...
function getcustomer()
    ...
    $('#custname').on('change',function(){
        ...
        var mem_id = this.value;
        $.ajax({
                url: "<?php echo base_url(); ?>MBTSale/get_old_bal",
                type: 'POST',
                data: {
                    vid: mem_id
                },
    ...
    { 
                  $.ajax({
                      ...
                      success: function(data) {
                          ...
                          $('#hsncode_id'+row).html(d.product_hsn);
                          $('#hsncode_id'+row).val(d.product_hsn);
