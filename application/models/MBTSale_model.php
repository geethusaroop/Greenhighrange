...
class MBTSale_model extends CI_Model
{
	...
	public function getSaleReport($param,$branch_id_fk)
	{
		...
		if ($product_num) {
			$this->db->like('tbl_master_branch_sale.invoice_number', $product_num);
		}
		...
		$this->db->select('*,COUNT(invoice_number) as slcount,SUM(sale_netamt) as total,sum(sale_quantity) as qty,(total_price-(sale_discount+sale_shareholder_discount)) as tprice,DATE_FORMAT(sale_date,\'%d/%m/%Y\') as sale_dates,tbl_master_branch_sale.sale_discount as discount');
	...
	public function get_sale_pdf($auto_invoice)
	{
		...
		$this->db->from('tbl_master_branch_sale');
		$this->db->where('sale_status', 1);
		$this->db->join('tbl_product', 'tbl_product.product_id = tbl_master_branch_sale.product_id_fk');
		$this->db->join('tbl_member', 'tbl_member.member_id = tbl_master_branch_sale.member_id_fk', 'left');
		$this->db->where('tbl_master_branch_sale.auto_invoice', $auto_invoice);
		$query = $this->db->get();
		$array = $query->result();
